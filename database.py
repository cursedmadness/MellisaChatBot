...
def create_table():
    ...
    if conn:
        try:
            ...
            ''')
            conn.commit()
            print("Таблицы 'users' и 'admins' созданы/проверены.")
        except Error as e:
...
def set_user_nickname(user_id: int, nickname: str):
    ...
    if conn:
        try:
            ...
            print(f"Ошибка установки ника: {e}")
        finally:
            conn.close()
...
def set_user_description(user_id: int, description: str):
    ...
    if conn:
        try:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE users SET Описание = ? WHERE user_id = ?", 
                (description, user_id)
            )
            conn.commit()
        except Error as e:
...
def get_user_description(user_id: int):
    """Получает описание пользователя."""
    conn = create_connection()
    if conn:
        try:
            cursor = conn.cursor()
...
def update_user_rate(user_id: int, rate: int):
    ...
    if conn:
        try:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE users SET Репутация = ? WHERE user_id = ?", 
                (rate, user_id)
            )
            conn.commit()
        except Error as e:
...
def increment_user_activity(user_id: int):
    """Увеличивает активность пользователя."""
    conn = create_connection()
    if conn:
        try:
            cursor = conn.cursor()
            ...
            print(f"Ошибка увеличения активности: {e}")
        finally:
            conn.close()
...
def get_profile_text(user_id: int) -> str:
    """Формирует текст анкеты пользователя."""
    profile_data = get_user_profile(user_id)
