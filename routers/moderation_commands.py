    ...
    if await add_warning(target_user_id, message.chat.id, reason, message.from_user.id):
        ...
        if count >= 3:
            try:
                ...
                await reset_user_rating(target_user_id)
                await message.answer(f"🚫 Гражданин {target_name} получил 3-е предупреждение и отправляется в бан на неделю! Рейтинг обнулен.")
            except Exception as e:
...
async def check_and_apply_auto_moderation(user_id: int, chat_id: int, bot: Bot):
    """
...
async def chat_banlist_command(message: Message, bot: Bot):
...
async def chat_redlist_command(message: Message, bot: Bot):
...
async def chat_blacklist_command(message: Message, bot: Bot):
...
async def dm_ban_reason(message: Message):
...
async def dm_mute_reason(message: Message):
