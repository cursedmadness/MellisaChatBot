    ...
    try:
        ...
        if not target_user_id:
            await message.answer(
                "Укажите Гражданина для назначения главой (реплай или @username)."
            )
            return

        if await is_admin(target_user_id):
            await message.answer(
                f"Гражданин {get_user_link(target_user_id)} уже всемогущ!"
            )
            return

        await add_admin(target_user_id, target_name)
        await message.answer(
            f"Гражданин {get_user_link(target_user_id, target_name)} теперь всемогущ!"
        )
...
async def _modify_rate(
    message: Message, command: CommandObject | None, multiply: int
) -> None:
    ...
    try:
        ...
        try:
            ...
            if rate_amount <= 0:
                await message.reply(
                    "Количество рейтинга должно быть положительным числом!"
                )
                return
            ...
            if len(parts) > 1:
                ...
                if not target_id:
                    # Пользователь указан, но не найден
                    await message.reply(
                        f"❌ Гражданин '{parts[1]}' не найден в системе."
                    )
                    return
                target_name = parts[1]
            ...
            if message.chat.type in ["group", "supergroup"]:
                await check_and_apply_auto_moderation(
                    target_id, message.chat.id, message.bot
                )
...
async def rating_history_command(
    ...
    try:
        ...
        if not history:
            await message.answer(
                f"В архивах Партии пока нет записей об изменении рейтинга гражданина {target_name}."
            )
            return
        ...
        plt.title(
            f"Динамика социального рейтинга: {target_name}",
            fontsize=14,
            fontweight="bold",
            pad=15,
        )
...
async def reset_rice_command(message: Message) -> None:
    ...
    try:
        await message.reply(
            "🔄 Начинаю сброс количества риса до 1 миски у всех пользователей..."
        )
        reset_count = await reset_all_rice_to_one()
        await message.reply(
            f"✅ Сброс риса завершен! Обновлено пользователей: {reset_count}"
        )
    except Exception as e:
...
async def reset_rating_command(message: Message) -> None:
    ...
    try:
        ...
        await message.reply(
            f"✅ Сброс рейтинга завершен! Обновлено граждан: {reset_count}"
        )
    except Exception as e:
...
async def broadcast_command(
    message: Message, bot: Bot, command: CommandObject = None
) -> None:
    """
    ...
    try:
        ...
        if not args_text:
            await message.reply(
                "❌ Вы не ввели текст для рассылки!\nФормат: <code>/рассылка Всем привет!</code>"
            )
            return

        await message.reply(
            "🔄 Начинаю массовую рассылку. Это может занять некоторое время..."
        )
        ...
        for i, user in enumerate(users):
            ...
            try:
                ...
                logger.debug(
                    f"Не удалось отправить рассылку пользователю {user_id}: {e}"
                )
                fail_count += 1
