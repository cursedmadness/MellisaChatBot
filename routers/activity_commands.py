...
from config import (
    ...
    DAILY_TOP_LIMIT,
    MONTHLY_TOP_LIMIT,
    DAILY_REPORT_TIME_HOUR,
    DAILY_REPORT_TIME_MINUTE,
    MONTHLY_REPORT_TIME_HOUR,
    MONTHLY_REPORT_TIME_MINUTE,
...
async def publish_daily_report(bot: Bot):
    ...
    logger.info(
        f"Ежедневная ведомость отправлена в {success_count} чатов, ошибок: {error_count}"
    )
...
async def publish_monthly_report(bot: Bot):
    ...
    logger.info(
        f"Ежемесячная ведомость отправлена в {success_count} чатов, ошибок: {error_count}"
    )
...
async def daily_report_task(bot: Bot):
    ...
    while True:
        try:
            ...
            logger.info(
                f"Следующая ежедневная ведомость через {seconds_until_next:.1f} секунд"
            )
    ...
    while True:
        try:
            ...
            target_time = datetime.combine(
                last_day.date(),
                time(MONTHLY_REPORT_TIME_HOUR, MONTHLY_REPORT_TIME_MINUTE, 0),
                tzinfo=timezone.utc,
            )
            ...
            logger.info(
                f"Следующая ежемесячная ведомость через {seconds_until_next:.1f} секунд"
            )
            await asyncio.sleep(seconds_until_next)
...
@activity_routers.message(
    F.text.lower().in_(["статистика", "стата", "сводка", "активность", "актив"])
)
