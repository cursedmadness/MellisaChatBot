...
async def set_nickname_handler(message: Message):
    ...
    set_user_nickname(user_id, nick)
    await message.answer(f"✅ Ваше учётное имя успешно изменено на {nick}")
...
async def set_description_handler(message: Message):
    ...
    if text.startswith('/set_description'):
        description = text[16:].strip()
    elif text.lower().startswith('сменить описание'):
        description = text[16:].strip()
    else:
        description = ""
...
# Роутер 'Анкета' - выводит анкету с данными в лс бота
@user_router.message(Command('anketa'))
@user_router.message(F.text.lower().in_(['анкета','досье']))
async def profile_handler(message: Message):
    user_id = message.from_user.id
...
async def reset_nickname_handler(message: Message):
    ...
    await message.answer("🗑️ Ваше имя успешно отправлено в ссылку.", parse_mode="Markdown")
...
async def show_my_description(message: Message):
    ...
    if description:
        ...
        await message.answer("У тебя пока нет описание. Можешь добавить его командой `/set_description`.", parse_mode="Markdown")
