...
from database import (
    ...
    get_all_admins, get_rate_status,
...
async def process_nickname(message: Message, state: FSMContext) -> None:
    try:
        ...
        if not new_nickname:
            await message.answer("❌ Ошибка: Имя не может быть пустым. Пожалуйста, напишите Ваш ник:")
            return
        
        await set_user_nickname(message.from_user.id, new_nickname)
        await message.answer(f"✅ Принято, {new_nickname}! Теперь Партии нужно знать Ваше местоположение для точного прогноза погоды. 📍\n"
...
async def process_city(message: Message, state: FSMContext) -> None:
    try:
        ...
        if not city:
            await message.answer("❌ Ошибка: Город не может быть пустым. Пожалуйста, напишите Ваш город:")
            return
        
        await set_user_city(message.from_user.id, city)
        await message.answer(f"✅ Город {city} закреплен за Вами. И последнее — расскажите немного о себе для Вашего досье. 📄\n"
...
@user_router.message(RegistrationStates.waiting_for_description, F.chat.type == "private")
async def process_description(message: Message, state: FSMContext) -> None:
    try:
        ...
        if not description:
            await message.answer("❌ Ошибка: Описание не может быть пустым. Пожалуйста, расскажите о себе:")
            return
        ...
        await message.answer(f"🎊 Поздравляем! Ваша регистрация завершена. Партия гордится Вами! 🇨🇳\n\n{profile_text}")
    except Exception as e:
...
@user_router.message(F.text.lower().startswith(('.ник ', 'ник ', '/set_nickname ')), F.chat.type == "private")
...
@user_router.message(F.text.lower().startswith(('.ник ', 'ник ')), F.chat.type != "private")
async def set_nickname_group_stub(message: Message) -> None:
    """Заглушка для попытки сменить ник в группе."""
    await message.reply("⚠️ Гражданин, изменение учётных данных (ника, описания, города) разрешено только в <b>личных сообщениях</b> бота. 🇨🇳")
...
@user_router.message(F.text.lower().startswith(('.описание ', 'описание ', '/set_description ')), F.chat.type == "private")
...
@user_router.message(F.text.lower().startswith(('.описание ', 'описание ')), F.chat.type != "private")
...
async def set_city_group_stub(message: Message):
    await message.reply("⚠️ Гражданин, изменение города разрешено только в <b>личных сообщениях</b> бота. 🇨🇳")
...
async def weather_command_handler(message: Message, command: CommandObject = None) -> None:
    ...
    try:
        ...
        if not city:
            await message.answer("📍 Пожалуйста, укажите город после команды или установите его в профиле через /set_city.\nПример: <code>/weather Москва</code>")
            return
...
@user_router.message(F.text.lower().in_(['удалить ник', 'удалить имя', '.удалить ник', '.удалить имя', 'сбросить ник']), F.chat.type == "private")
async def reset_nickname_handler(message: Message) -> None:
    ...
    try:
        ...
        await message.answer(f"🗑️ Ваше имя успешно отправлено в ссылку. Теперь Вы снова {first_name}.")
    except Exception as e:
        logger.error(f"Ошибка в reset_nickname_handler: {e}")
        await message.answer("Ошибка при удалении ника.")

@user_router.message(F.text.lower().in_(['удалить ник', 'удалить имя', '.удалить ник', '.удалить имя']), F.chat.type != "private")
async def reset_nickname_group_stub(message: Message):
    await message.reply("⚠️ Гражданин, удаление ника разрешено только в <b>личных сообщениях</b> бота. 🇨🇳")

@user_router.message(Command('delete_description'), F.chat.type == "private")
@user_router.message(F.text.lower().in_(['удалить описание', 'очистить описание', '.удалить описание', '.очистить описание']), F.chat.type == "private")
...
async def clear_description_group_stub(message: Message):
    await message.reply("⚠️ Гражданин, удаление описания разрешено только в <b>личных сообщениях</b> бота. 🇨🇳")
